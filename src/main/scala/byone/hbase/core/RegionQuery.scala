 ...
 */
class RegionQuery(startKey: Int,filter: Filter, range: List[Array[Byte]],items: List[String])
  ...
  def call(): RDD[(ImmutableBytesWritable,Result)] = {
    ...
    val futures = for (i <- 0 until regions) yield
    {
      ...
      pool.submit(new RegionSubQuery(DatePoint.ScanToString(sn)))
 ...
 * @param scanString
 */
class RegionSubQuery(scanString: String) extends Callable[RDD[(ImmutableBytesWritable,Result)]]
{
  def call() : RDD[(ImmutableBytesWritable,Result)] =
  {
