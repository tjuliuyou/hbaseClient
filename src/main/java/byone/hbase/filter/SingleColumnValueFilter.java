 ...
 * <p>
 * To filter based on the value of all scanned columns,
 */
...
public class SingleColumnValueFilter extends FilterBase {
     ...
     */
    public SingleColumnValueFilter(final byte [] family, final byte [] qualifier,
                                   final CompareFilter.CompareOp compareOp, final ByteArrayComparable comparator) {
    ...
    protected SingleColumnValueFilter(final byte[] family, final byte[] qualifier,
                                      final CompareFilter.CompareOp compareOp, ByteArrayComparable comparator, final boolean filterIfMissing,
    ...
    public static Filter createFilterFromArguments(ArrayList<byte []> filterArguments) {
        ...
        byte [] family = ByParseFilter.removeQuotesFromByteArray(filterArguments.get(0));
        byte [] qualifier = ByParseFilter.removeQuotesFromByteArray(filterArguments.get(1));
        CompareOp compareOp = ByParseFilter.createCompareOp(filterArguments.get(2));
        ByteArrayComparable comparator = ByParseFilter.createComparator(
                ByParseFilter.removeQuotesFromByteArray(filterArguments.get(3)));
        ...
        if (filterArguments.size() == 6) {
            boolean filterIfMissing = ByParseFilter.convertByteArrayToBoolean(filterArguments.get(4));
            boolean latestVersionOnly = ByParseFilter.convertByteArrayToBoolean(filterArguments.get(5));
    ...
    FilterProtos.SingleColumnValueFilter convert() {
        ...
        builder.setComparator(ByProtobufUtil.toComparator(this.comparator));
    ...
    public static SingleColumnValueFilter parseFrom(final byte [] pbBytes)
        ...
        try {
            comparator = ByProtobufUtil.toComparator(proto.getComparator());
